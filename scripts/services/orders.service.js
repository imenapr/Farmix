...
export async function placeOrder(buyerId, listingId, quantity) {
  ...
  await createNotification({
    ...
    message: t("service.newOrderMessage", {
      default: `${user.name || "A buyer"} ordered ${qty} ${listing.unit} of "${listing.title}".`,
      buyer: user.name || t("common.buyer"),
      qty: String(qty),
      unit: listing.unit,
      title: listing.title,
    }),
    metadata: { orderId: order.id, listingId, buyerId: user.id, quantity: qty },
  });
